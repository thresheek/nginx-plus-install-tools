...
extract() {
    ...
    if [ -x $TMPDIR/usr/sbin/nginx ]; then
        ...
        if [ $TARGETVER -ge 33 ]; then
            ...
            echo "mgmt { license_file $ABSPATH/etc/nginx/license.jwt; }" >> $ABSPATH/etc/nginx/nginx.conf
        fi
...
upgrade() {
    ...
    if [ -x $TMPDIR/usr/sbin/nginx ]; then
        ...
        TARGETVER=$($ABSPATH/usr/sbin/nginx -v 2>&1 | cut -d '(' -f 2 | cut -d ')' -f 1 | cut -d'-' -f 3 | tr -d 'r')
        if [ $TARGETVER -ge 33 ]; then
            if ! $ABSPATH/usr/sbin/nginx -p $ABSPATH/etc/nginx -c nginx.conf -T 2>&1 | grep 'license_file' | grep -vE '^(.*)#.*license_file' >/dev/null; then
                echo "mgmt { license_file $ABSPATH/etc/nginx/license.jwt; }" >> $ABSPATH/etc/nginx/nginx.conf
            fi
        fi
        if [ $TARGETVER -ge 31 -a $TARGETVER -lt 33 ]; then
